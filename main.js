...
$('.send-btn').click(function() {
    ...
    if(answer_text.length != 0 && $('.chat-container').children().length == 0 ) {
        // print today's date
    ...
    } else if(answer_text.length != 0 && today != $('.chat-container .date:last').text() && ( $('.chat-container').children().last().hasClass('message-wrapper') || $('.chat-container').children().last().hasClass('answer-wrapper') ) ) {
        // print today's date
        printTodayDate(today);
        // print user's message
        printAnswer(answer_text);
    ...
    // if the last message in chat is a message from the user
    } else if(answer_text.length != 0 && $('.chat-container').children().last().hasClass('answer-wrapper')) {
        ...
        showMic();
    // if the last message in chat is a message from the user
    } else if(answer_text.length != 0 && $('.chat-container').children().last().hasClass('message-wrapper')) {
        // print user's message
        printAnswer(answer_text);
        // print timestamp
        printTimeStampAnswer(time);
